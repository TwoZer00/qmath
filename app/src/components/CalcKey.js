...
export default function CalcKey({ label, icon, variant = 'num', onPress, disabled, wide, tall, style }) {
  ...
  return (
    <Animated.View style={[{ transform: [{ scale }] }, style]}>
      <TouchableOpacity style={keyStyle} onPress={handlePress} disabled={disabled} activeOpacity={1}>
        {icon && label
          ? <><Icon name={icon} size={16} color={variant === 'action' || variant === 'secondary' ? '#fff' : colors.keyText} /><Text style={[textStyle, { marginLeft: 6 }]}>{label}</Text></>
          : icon
          ? <Icon name={icon} size={22} color={variant === 'action' || variant === 'secondary' ? '#fff' : colors.keyText} />
...
const s = StyleSheet.create({
  ...
  // Categorías
  num:       { backgroundColor: '#2e3245', borderColor: '#3a3f55', borderWidth: 1, borderBottomColor: '#1a1d2a' },
  fn:        { backgroundColor: '#3a3f55', borderColor: '#4a5068', borderWidth: 1, borderBottomColor: '#252836' },
  action:    { backgroundColor: '#c0392b', borderColor: '#e74c3c', borderWidth: 1, borderBottomColor: '#7b241c' },
